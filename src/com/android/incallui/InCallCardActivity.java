...
public class InCallCardActivity extends Activity {
	...
	protected void onCreate(Bundle icicle) {
		...
		vg.setTranslationY(getResources().getDimensionPixelSize(
				R.dimen.incoming_call_card_height));
	...
	/**
	 * Starts a query for more contact data for the save primary and secondary
	 * calls.
	 */
	private void startContactInfoSearch(final CallIdentification identification) {
		final ContactInfoCache cache = ContactInfoCache
				.getInstance(InCallCardActivity.this);
		...
		cache.findInfo(identification, true, new ContactInfoCacheCallback() {
			...
			public void onContactInfoComplete(int callId,
				...
				mNameTextView.setText(entry.name == null ? entry.number
						: entry.name);
				if (entry.personUri != null) {
					CallerInfoUtils.sendViewNotification(
							InCallCardActivity.this, entry.personUri);
				}
			}
			...
			public void onImageLoadComplete(int callId, ContactCacheEntry entry) {
				if (entry.photo != null) {
					...
					AnimationUtils.startCrossFade(mContactImage, current,
							entry.photo);
				}
			}
		});
	...
	public void onBackPressed() {
		...
		// Fix for http://jira.omnirom.org/browse/OMNI-296
	}
}